				  ...
				  unsigned char* Out,unsigned datalen,const unsigned char* Key,unsigned keylen)
{
	...
	if(bMode == ECB)	//ECB模式
	{
		if(nKey	==	1)	//单Key
		{
			...
			if (bType == DECRYPT)
			{
				int ilen = strlen((char *)jiemi);
				//将char转换成ascii
				int backpad = jiemi[ilen - 1];
				memset(jiemi + ilen - backpad , 0, backpad);
			}
...
std::string CDES::base64_encode(unsigned char const* bytes_to_encode, unsigned int in_len) {
	std::string ret;
	int i = 0;
	int j = 0;
...
std::string CDES::base64_decode(std::string const& encoded_string) 
{
...
inline bool CDES::is_base64(unsigned char c) 
{
